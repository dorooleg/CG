...
struct TGAColor final {
  ...
  TGAColor() noexcept : val(0), bytespp(1) {}
  ...
  TGAColor(const TGAColor& c) noexcept : val(c.val), bytespp(c.bytespp) {}

  TGAColor(const unsigned char* p, int bpp) noexcept : val(0), bytespp(bpp) {
    for (int i = 0; i < bpp; i++) {
      raw[i] = p[i];
    }
  }
